...
function buildCleanConsoleLog(
  ...
  if (message === "Official outstanding monthly buckets prepared") {
    return `✅ Outstanding buckets | total=${formatLogNumber(details?.totalRows)} | buckets=${details?.buckets?.length || 0} | dup=${formatLogNumber(details?.duplicates)}`;
  }
...
async function getCompaniesForSync(): Promise<TallyCompanyForTransactions[]> {
  try {
    ...
    const companies = parseCompanies(String(companiesXml || ""));

    if (companies.length) {
      addEvent("info", "Loaded Tally companies found", {
    ...
    addEvent(
      "warn",
      "Could not read loaded Tally companies, env fallback used",
      {
  ...
  if (envCompanies.length) {
    addEvent("info", "Using TALLY_COMPANIES fallback", {
      companies: envCompanies.map((company) => company.name),
    });
  }
...
function isSalesVoucher(record: any) {
  const type = normalizeVoucherType(
    record?.voucherType || record?.voucher_type_name || record?.voucherTypeName,
  );

  return Boolean(type && type.includes("sales") && !type.includes("order"));
}
...
function isPurchaseVoucher(record: any) {
  ...
  return Boolean(type && type.includes("purchase") && !type.includes("order"));
}
...
function applyOfficialOutstandingDateGuard(input: {
  ...
  return (input.records || []).filter((record) => {
    ...
    if (billDate < input.fromDate) return false;
...
async function pushOfficialOutstandings(input: {
  ...
  historicalTransactionsStatus.summary.pulledRecords +=
    receivableRows.length + payableRows.length;
  historicalTransactionsStatus.summary.skippedDuplicateRecords +=
    unique.duplicateCount;

  const monthGroups = groupOutstandingRecordsByMonth(unique.records);

  addEvent("info", "Official outstanding monthly buckets prepared", {
    company: company.name,
    totalRows: unique.records.length,
    duplicates: unique.duplicateCount,
    buckets: monthGroups.map((bucket) => ({
  ...
  for (const bucket of monthGroups) {
    historicalTransactionsStatus.activeRange = {
      fromDate: bucket.fromDate,
      toDate: bucket.toDate,
    };

    const receivableCount = bucket.records.filter(
      (row) => row.billType === "receivable" || row.bill_type === "receivable",
    ).length;
    const payableCount = bucket.records.filter(
      (row) => row.billType === "payable" || row.bill_type === "payable",
    ).length;
    const amount = bucket.records.reduce(
      (sum, row) => sum + Number(row.pendingAmount || row.pending_amount || 0),
      0,
    );

    addEvent("info", "Outstanding range collected", {
      company: company.name,
      fromDate: bucket.fromDate,
      toDate: bucket.toDate,
    ...
    const result = await pushOutstandingsToCrm(bucket.records, {
      companyName: company.name,
      companyGuid: company.guid,
      syncMode: "historical",
      fromDate: bucket.fromDate,
    ...
    const uploaded = Number(
      result?.uploadedRecords || bucket.records.length || 0,
    );
    uploadedTotal += uploaded;
    historicalTransactionsStatus.summary.uploadedRecords += uploaded;

    addEvent("info", "Outstanding range pushed", {
      company: company.name,
      fromDate: bucket.fromDate,
      toDate: bucket.toDate,
      pushed: bucket.records.length,
      uploaded,
      batches: result?.totalBatches ?? null,
      source: "official-report",
    });
    ...
    results.push({
      ...
      collected: bucket.records.length,
      uploaded,
      result,
    });
  }
  ...
  return {
    module: "outstandings",
    parsed: receivableRows.length + payableRows.length,
    receivableRows: receivableRows.length,
    payableRows: payableRows.length,
    pushed: unique.records.length,
...
async function syncCompanyTransactions(input: {
  ...
  historicalTransactionsStatus.summary.rangesTotal +=
    ranges.length *
    modules.filter((m) => m === "sales-vouchers" || m === "purchase-vouchers")
      .length;
  ...
  for (const dateRange of ranges) {
    addEvent("info", "Transaction range started", {
      ...
      modules: modules.filter(
        (m) => m === "sales-vouchers" || m === "purchase-vouchers",
      ),
    });
